...
namespace Com.LuisPedroFonseca.ProCamera2D
{
    ...
    public class ProCamera2D : MonoBehaviour
    {
        ...
        public Rect CameraWindowRect = new Rect(0f, 0f, 0f, 0f);
        ...
        public bool UseGeometryBoundaries;
        ...
        public Vector3 CameraPosition { get { return _transform.localPosition; } }

        float _originalCameraDepthSign;

        public Vector3? ExclusiveTargetPosition;
        public Vector3 CameraTargetPositionDuringExclusive;
        ...
        public void UpdateScreenSize(float newSize, float duration = 0)
        {
            if (duration > 0)
            {
                StartCoroutine(UpdateScreenSizeRoutine(newSize, duration));
                return;
            }
            ...
            {
                var distance = newSize / Mathf.Tan(GameCamera.fieldOfView * 0.5f * Mathf.Deg2Rad);
                _transform.localPosition = VectorHVD(Vector3H(_transform.localPosition), Vector3V(_transform.localPosition), distance * _originalCameraDepthSign);
            }
        ...
        void LimitSizeAndPositionToNumericBoundaries()
        {
            ...
            if (UseRightBoundary && UseLeftBoundary && ScreenSizeInWorldCoordinates.x > cameraBounds.x)
            {
                UpdateScreenSize(cameraBounds.x / GameCamera.aspect / 2);
                IsCameraSizeBounded = true;
            }

            if (UseTopBoundary && UseBottomBoundary && ScreenSizeInWorldCoordinates.y > cameraBounds.y)
            {
                UpdateScreenSize(cameraBounds.y / 2);
                IsCameraSizeBounded = true;
            }
            ...
            if (UseLeftBoundary && newPosH - ScreenSizeInWorldCoordinates.x / 2 < LeftBoundary)
            {
                newPosH = LeftBoundary + ScreenSizeInWorldCoordinates.x / 2;
                IsCameraPositionHorizontallyBounded = true;
            } else if (UseRightBoundary && newPosH + ScreenSizeInWorldCoordinates.x / 2 > RightBoundary)
            {
                newPosH = RightBoundary - ScreenSizeInWorldCoordinates.x / 2;
                IsCameraPositionHorizontallyBounded = true;
            }

            // Check movement in the vertical dir
            var newPosV = Vector3V(_transform.localPosition);
            if (UseBottomBoundary && newPosV - ScreenSizeInWorldCoordinates.y / 2 < BottomBoundary)
            {
                newPosV = BottomBoundary + ScreenSizeInWorldCoordinates.y / 2;
                IsCameraPositionVerticallyBounded = true;
            } else if (UseTopBoundary && newPosV + ScreenSizeInWorldCoordinates.y / 2 > TopBoundary)
            {
                newPosV = TopBoundary - ScreenSizeInWorldCoordinates.y / 2;
                IsCameraPositionVerticallyBounded = true;
            }

            // Set to the new position
            if (IsCameraPositionHorizontallyBounded || IsCameraPositionVerticallyBounded)
                _transform.localPosition = VectorHVD(newPosH, newPosV, Vector3D(_transform.localPosition));
        }
        ...
        public void MoveCameraInstantlyToPositionKlaus()
        {
            ...
            _cameraTargetPosition = VectorHVD(Vector3H(targetPos), Vector3V(targetPos), Vector3D(_transform.localPosition));
