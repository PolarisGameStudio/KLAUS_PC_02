...
namespace LetterboxCamera
{
    ...
    public class ForceCameraRatio : MonoBehaviour
    {
        ...
        public void CalculateAndSetAllCameraRatios () {
            ...
            targetAspect = 1.5f; //added line just for resolutions lower than 4:3
            float currentAspect = ((float)Screen.width) / ((float)Screen.height);

           // Debug.Log("target aspect " + targetAspect + " current aspect " + currentAspect);

            if(currentAspect<targetAspect) //only happens if the current aspect ratio is around 800x600
            { 
