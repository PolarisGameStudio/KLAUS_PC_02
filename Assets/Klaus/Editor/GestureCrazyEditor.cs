...
namespace TouchScript.Editor.Gestures
{
	...
	internal class GestureCrazyEditor : UnityEditor.Editor
	{
		...
		private void drawAdvanced()
		{
			...
			var expanded = GUICrazyElements.BeginFoldout(advanced.isExpanded, new GUIContent("Advanced", TEXT_FRIENDLY_HEADER));
			...
			if (expanded)
			{
				GUILayout.BeginVertical(GUICrazyElements.FoldoutStyle);
				drawSendMessage();
				drawCombineTouches();
				drawRequireToFail();
				GUILayout.EndVertical();
			}
			GUICrazyElements.EndFoldout();
		}
		
		private void drawFriendlyGestures()
		{
			EditorGUI.BeginChangeCheck();
			var expanded = GUICrazyElements.BeginFoldout(friendlyGestures.isExpanded, new GUIContent(string.Format("Friendly gestures ({0})", friendlyGestures.arraySize), TEXT_FRIENDLY_HEADER));
			if (EditorGUI.EndChangeCheck())
			{
		...
		private void drawGestureList(SerializedProperty prop, Action<SerializedProperty, Gesture> addGesture, Func<SerializedProperty, int, Gesture> removeGestureAt)
		{
			...
			for (int i = 0; i < prop.arraySize; i++)
			{
				...
				{
					Rect rect = EditorGUILayout.BeginHorizontal(GUICrazyElements.BoxStyle, GUILayout.Height(23));
					EditorGUILayout.LabelField(string.Format("{0} @ {1}", gesture.GetType().Name, gesture.name), GUICrazyElements.BoxLabelStyle, GUILayout.MinWidth(100), GUILayout.ExpandWidth(true));
			...
			Rect dropArea = GUILayoutUtility.GetRect(0.0f, 50.0f, GUICrazyElements.BoxStyle,
			                                         GUILayout.ExpandWidth(true));
			GUI.Box(dropArea, "Drag a Gesture Here", GUICrazyElements.BoxStyle);
